...
export default function Home() {
  ...
  const createTempMail = async () => {
    ...
    let result = null;
    try {
      const body = JSON.stringify({ email_address: fullEmail });
      const headers = await buildAuthHeaders({
        path: "/.netlify/functions/createTempMail",
        payload: body,
      });
      const response = await fetch("/.netlify/functions/createTempMail", {
        method: "POST",
        headers: { "Content-Type": "application/json", ...headers },
        body,
      });
      if (response.status === 409) {
        toast.error("Email sudah ada. Silakan login dengan token.");
        setIsCreating(false);
        return;
      }
      if (!response.ok) {
            ...
            {[
              {
                icon: Zap,
                title: "Instan",
                desc: "Buat email dalam hitungan detik",
                color: "bg-cyan-300",
              },
              {
                icon: Shield,
                title: "Aman",
                desc: "Lindungi privasi Anda",
                color: "bg-pink-300",
              },
              {
                icon: Clock,
                title: "Sementara",
                desc: "Otomatis kedaluwarsa",
                color: "bg-lime-300",
              },
            ].map((feature, index) => (
              <motion.div
            ...
            {savedMails.length === 0 ? (
              <div className="bg-white border-4 border-black p-6 text-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                ...
                <p className="text-sm text-gray-600">
                  Email yang dibuat akan muncul di sini.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {pagedMails.map((mail) => (
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-600 mt-1">
                      Dibuat:{" "}
                      {new Date(mail.created_date).toLocaleString("id-ID")}
                    </p>
                ...
                <div className="flex items-center justify-between pt-2">
                  <button
                    onClick={() =>
                      setSavedPage((prev) => Math.max(1, prev - 1))
                    }
                  ...
                  <button
                    onClick={() =>
                      setSavedPage((prev) => Math.min(totalPages, prev + 1))
                    }
